...
def find_matching_event(filename, events):
    ...
    for event in events:
        event_keywords = event['mot_clefs_dalle']
    ...
    for event, score in matches[:3]:
        ...
        print(f"Score: {score}")
        print(f"Mots-clés DALLE: {event['mot_clefs_dalle']}")
        print()
...
def update_event_with_image(event_id, image_url):
    response = supabase.table("evenements").update({"illustration_url": image_url}).eq("id", event_id).execute()
    if response.data:
        print(f"Événement {event_id} mis à jour avec l'URL de l'image")
        logging.info(f"Événement {event_id} mis à jour avec l'URL de l'image")
    else:
        print(f"Erreur lors de la mise à jour de l'événement: {response}")
        logging.error(f"Erreur lors de la mise à jour de l'événement: {response}")

# Récupération des événements en attente d'illustration de Supabase
response = supabase.table("evenements").select("id", "mot_clefs_dalle", "titre").eq("illustration_url", "attente").execute()
events_waiting_illustration = response.data

print(f"Nombre d'événements en attente d'illustration : {len(events_waiting_illustration)}")
...
for filename in os.listdir(image_folder):
    if filename.endswith(('.webp', '.jpg', '.png')):
        ...
        if matching_event:
            ...
            print(f"Titre: {matching_event['titre']}")
            print(f"Mots-clés DALLE: {matching_event['mot_clefs_dalle']}")
            ...
            if confirmation.lower() == 'o':
                ...
                print(f"Image {filename} associée à l'événement")
            else:
                print(f"Association non confirmée pour {filename}")
        else:
            print(f"\nAucun événement correspondant trouvé au-dessus du seuil pour {filename}")
            manual_input = input("Voulez-vous entrer manuellement les mots-clés ? (o/n) : ")
            if manual_input.lower() == 'o':
                search_terms = input("Entrez les mots-clés en français séparés par des espaces : ")
                ...
                if manual_matching_event:
                    ...
                    print(f"Titre: {manual_matching_event['titre']}")
                    print(f"Mots-clés DALLE: {manual_matching_event['mot_clefs_dalle']}")
                    confirmation = input("Cette correspondance est-elle correcte ? (o/n): ")
                    if confirmation.lower() == 'o':
                        image_url = upload_image(full_path, filename)
                        if image_url:
