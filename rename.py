...
def clean_filename(filename):
    ...
    # Réduit le nom à un maximum de mots
    words = clean_name.split('_')
    clean_name = '_'.join(words[:5])  # Limite à 5 mots
    
    # Limite la longueur du nom de fichier (en gardant de la place pour l'ID unique et l'extension)
    max_length = 60 - 9 - len(ext)  # 9 pour l'ID unique et le tiret bas, 60 caractères au total
